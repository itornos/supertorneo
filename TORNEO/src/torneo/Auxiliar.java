...
public class Auxiliar {
	...
	public String[] ordenarE(String[] orden){
		Menus menu = new Menus();
		for(int i = orden.length-1 ;i>0;i--){
			for(int j =0 ;j<orden.length-1;j++){

				int primero = buscarNombreE(Registro.clubs.size(),orden[j]);
				int segundo = buscarNombreE(Registro.clubs.size(),orden[j+1]);

				if(Registro.clubs.get(primero).puntos<Registro.clubs.get(segundo).puntos){

					orden[j] = Registro.clubs.get(segundo).club;
					orden[j+1] = Registro.clubs.get(primero).club;
